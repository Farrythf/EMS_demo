...
bool TimerEngine::StopEngine()
{
	...
	mpThread = NULL;
	mTimerDetails.clear();
	while (false == mTimerCallBacks.empty())
	{
		mTimerCallBacks.pop();
	}
...
bool TimerEngine::AddTimer(ITimerEngineCallBackPtr pCallBack, DWORD TimerID, DWORD Interval /*= 1000*/, DWORD Times /*= 1*/, DWORD Param /*= 0 */)
{
	if (NULL == pCallBack)
	{
		assert(false);
